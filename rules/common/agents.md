...
作業中にスコープ外の課題（別件のバグ・古いドキュメント・見つけたセキュリティ上の懸念など）に
気づいたら、いまの作業を止めずに `spawn_task` でチップとして切り出す。チップはユーザーが 1 クリックで
別セッションとして開始できる。

**切り出す前に、いまの変更と競合しないかを判定する**（判定条件は [work-memory.md](./work-memory.md)
「作業中に別の課題を見つけたとき」）。競合するものをチップにすると、別セッションが同じファイルを
並行して触ることになる。

- **そのチップの内容に自分で着手したら、着手した時点で `dismiss_task` を呼んで取り下げる。**
  完了時ではなく着手時に消すこと。残したままだとユーザーがチップを押し、別セッションが同じ修正を
  並行して始めてしまう（ワークツリーとブランチが二重にでき、あとで衝突を裁く羽目になる）
- 取り下げるときは理由を添える（例:「このセッションで実装済み」「別チップに統合した」）
- 提案が古くなった・別の変更に取って代わられた場合も同様に取り下げる。差し替えるときは
  **新しいチップを作ってから**古い方を取り下げる
- ユーザーが既に開始・却下したチップは取り下げられない。その旨が返ってきたら再試行せず、
  自分の作業と重複していないかユーザーに確認する
