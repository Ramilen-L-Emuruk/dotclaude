...
9. **main の取り込みとリリース待ちへの登録**
    ...
    - **ブランチ名を `worktree/*` から `ready/*` へ改名する前に、ユーザーに確認を取る**（改名は「この内容でリリースしてよい」という宣言にあたる。詳細は [worktree.md](./worktree.md)）
...
| ユーザーの言葉 | 意味 |
|---|---|
| 「コミットして」 | 手順 8〜9 のうちコミットと main の取り込みまで。**`ready/` 改名の手前で止まり、改めて確認する** |
| 「リリース待ちに入れて」 | `ready/` へ改名し、実装セッションを終える |
| 「リリースして」 | 続けて手順 10（リリース）へ進む |
